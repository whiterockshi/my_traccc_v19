...
struct seedfilter_config {
    ...
    float deltaRMin = 20.f * unit<float>::mm;
    ...
    float good_spB_weight_increase = 0.f;
    float good_spT_max_radius = 150.f * unit<float>::mm;
    float good_spT_weight_increase = 0.f;

    // bottom sp cut
    // *SS* It should give always single_seed_cut=true.
    float good_spB_min_weight = -1000.f;

    // seed cut
    float seed_min_weight = -1000.f;
    float spB_min_radius = 0.f * unit<float>::mm;
};
