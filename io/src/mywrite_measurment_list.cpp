...
void mylist_meas_in_tracks(std::size_t event, std::string_view directory,
	     traccc::track_state_container_types::const_view tracks_view,
	     std::string_view device, std::string_view step){
  ...
  out_file<<"trackId,measurements"
	    <<std::endl;

  traccc::track_state_container_types::const_device tracks(tracks_view);
  const unsigned int n_tracks = tracks.size();
  for(unsigned int i=0; i<n_tracks ; i++){
    auto const & [fit, states] = tracks.at(i);
    out_file<<i;

    for(const auto& st: states){
      out_file<<","<<st.get_measurement().measurement_id;
    }
